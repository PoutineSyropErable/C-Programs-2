...
void object_screen_clip(Object3D *object_3d)
{
    for (int i = 0; i < object_3d->pFace_matrix->len; i++)
    {
        object_3d->pFace_list[i]->ss_p1;
    }
}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR pCmdLine, int nCmdShow)
{
    ...
    float player_rot_x = 0, player_rot_y = 0, player_rot_z = 0;
    ...
    while (!quit)
    {
        ...
        //It moves you in worldspace coordinates, not view space coordinates. 
        // I'd need to calc the rotation matrix here, and take the x,y,z component. 
        //then do add(player_pos_global, mouvement_vector)... I can't be fucked
        //I did that in python, I'm happy my crappy C port works.
        static int keyboard_x = 0, keyboard_y = 0;
        if (keyboard[VK_RIGHT] || keyboard['D'])
        {
            *(player_pos_global.x) -= (float)0.01;
        }
        ...
        {
            *(player_pos_global.y) -= (float)0.01;
        }
        ...
        *(player_rot_global.y) -= mouse_sensitivity * (float)(mouse_rel.x);
        *(player_rot_global.x) += mouse_sensitivity * (float)(mouse_rel.y);
...
LRESULT CALLBACK WindowProcessMessage(HWND window_handle, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_MOUSEMOVE:
    {
        ...
        mouse_rel.x = mouse.x - old_mouse.x;
        mouse_rel.y = mouse.y - old_mouse.y;
    }
    break;
